...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, OBB2D> : ModifyBase<ModifyDerive<Der, OBB2D>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(OBB2D, computeAxes, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(OBB2D, getBoundingRect, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(OBB2D, init, , cocos2d::CCPoint, float, float, float)
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0x6dd70, Thiscall, OBB2D, orderCorners, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(OBB2D, overlaps, , OBB2D*)
