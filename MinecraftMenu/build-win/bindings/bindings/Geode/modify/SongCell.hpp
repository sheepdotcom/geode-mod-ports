...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, SongCell> : ModifyBase<ModifyDerive<Der, SongCell>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(SongCell, init, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(SongCell, draw, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION(base::get() + 0xb3990, Thiscall, SongCell, loadFromObject, SongObject*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(SongCell, onClick, , cocos2d::CCObject*)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(SongCell, updateBGColor, , int)
		}
	};
}
