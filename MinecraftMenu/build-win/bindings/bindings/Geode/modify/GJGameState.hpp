...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, GJGameState> : ModifyBase<ModifyDerive<Der, GJGameState>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(GJGameState, controlTweenAction, , int, int, GJActionCommand)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(GJGameState, getGameObjectPhysics, , GameObject*)
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(GJGameState, updateTweenAction, , float, int)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(GJGameState, updateTweenActions, , float)
		}
	};
}
