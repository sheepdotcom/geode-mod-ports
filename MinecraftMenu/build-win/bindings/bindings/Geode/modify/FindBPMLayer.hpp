...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, FindBPMLayer> : ModifyBase<ModifyDerive<Der, FindBPMLayer>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FindBPMLayer, create, , int)
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FindBPMLayer, calculateBPM, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FindBPMLayer, init, , int)
		}
	};
}
