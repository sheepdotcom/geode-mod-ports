...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, FileSaveManager> : ModifyBase<ModifyDerive<Der, FileSaveManager>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FileSaveManager, sharedState, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FileSaveManager, init, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FileSaveManager, firstLoad, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FileSaveManager, getStoreData, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(FileSaveManager, loadDataFromFile, , char const*)
		}
	};
}
