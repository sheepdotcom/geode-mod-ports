...
namespace geode::modifier {
	...
	struct ModifyDerive<Der, CCCountdown> : ModifyBase<ModifyDerive<Der, CCCountdown>> {
		...
		void apply() override {
			...
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(CCCountdown, create, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(CCCountdown, init, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(CCCountdown, setOpacity, , unsigned char)
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(CCCountdown, lapFinished, )
			GEODE_APPLY_MODIFY_FOR_FUNCTION_ERROR(CCCountdown, startTimerWithSeconds, , float, cocos2d::SEL_CallFunc, cocos2d::CCNode*)
		}
	};
}
