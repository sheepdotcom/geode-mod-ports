        ...
        if (auto result = event->getValue()) {
            ...
            if (path.string().ends_with(".json")) {
                ...
                try {
                    ...
                    std::string error;
                    geode::Result<matjson::Value, matjson::ParseError> parsedData = matjson::parse(data);
                    std::optional<matjson::Value> optValue = parsedData.unwrap();
                    ...
                    if (!this->m_isWeb) {
                        for(int it = 0; it < this->m_jsonSets["shapes"].asArray().unwrap().size(); it++) {
                            if(std::find(this->m_supportedObjsDesktop.begin(), this->m_supportedObjsDesktop.end(), this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<int, std::string>("type").unwrap()) != this->m_supportedObjsDesktop.end() && this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<float, std::string>("score").unwrap() > 0) {
                                this->m_objsCount++;
                            }
                        }
                    } else {
                        for(int it = 0; it < this->m_jsonSets.asArray().unwrap().size(); it++) {
                            if(std::find(this->m_supportedObjsWeb.begin(), this->m_supportedObjsWeb.end(), this->m_jsonSets.asArray().unwrap().at(it).get<int, std::string>("type").unwrap()) != this->m_supportedObjsWeb.end() && this->m_jsonSets.asArray().unwrap().at(it).get<float, std::string>("score").unwrap() > 0) {
                                this->m_objsCount++;
                            }
                        }
                    }
...
void ImportPopup::convert() {
    try {
        ...
        if (!this->m_isWeb) {
            for (int it = 0; it < this->m_jsonSets["shapes"].asArray().unwrap().size(); it++) {
                if(this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<float, std::string>("score").unwrap() > 0) {
                    if (this->m_jsonSets["shapes"].asArray().unwrap().at(it)["color"].asArray().unwrap().at(3).asInt().unwrap() < 255 && std::find(this->m_supportedObjsDesktop.begin(), this->m_supportedObjsDesktop.end(), this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<int, std::string>("type").unwrap()) != this->m_supportedObjsDesktop.end() && this->m_jsonSets["shapes"].asArray().unwrap().at(it) != this->m_jsonSets["shapes"].asArray().unwrap().at(0) && !isTransparent) {
                        isTransparent = true;
                        transparency = (float)this->m_jsonSets["shapes"].asArray().unwrap().at(it)["color"].asArray().unwrap().at(3).asInt().unwrap() / 255;
                    }
                    if (this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<int, std::string>("type").unwrap() == this->m_supportedObjsDesktop[0]) {
                        ...
                        this->m_objsString << ";";
                        z_order++;
                    } else if (this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<int, std::string>("type").unwrap() == this->m_supportedObjsDesktop[1]) {
                        this->m_objsString << "1," << this->m_circle_id;
                        this->m_objsString << ",2," << (float)this->m_jsonSets["shapes"].asArray().unwrap().at(it)["data"].asArray().unwrap().at(0).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionX();
                        this->m_objsString << ",3," << (float)this->m_jsonSets["shapes"].asArray().unwrap().at(it)["data"].asArray().unwrap().at(1).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionY();
                        ...
                        this->m_objsString << ";";
                        z_order++;
                    } else if (this->m_jsonSets["shapes"].asArray().unwrap().at(it).get<int, std::string>("type").unwrap() == this->m_supportedObjsDesktop[2]) {
                        this->m_objsString << "1," << this->m_circle_id;
                        this->m_objsString << ",2," << (float)this->m_jsonSets["shapes"].asArray().unwrap().at(it)["data"].asArray().unwrap().at(0).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionX();
                        this->m_objsString << ",3," << (float)this->m_jsonSets["shapes"].asArray().unwrap().at(it)["data"].asArray().unwrap().at(1).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionY();
                        this->m_objsString << ",128," << (float)this->m_jsonSets["shapes"].asArray().unwrap().at(it)["data"].asArray().unwrap().at(2).asDouble().unwrap() * this->m_drawScale / 4;
            ...
            for (int it = 0; it < this->m_jsonSets.asArray().unwrap().size(); it++) {
                if(this->m_jsonSets.asArray().unwrap().at(it).get<float, std::string>("score").unwrap() > 0) {
                    if (this->m_jsonSets.asArray().unwrap().at(it)["color"].asArray().unwrap().at(3).asInt().unwrap() < 255 && std::find(this->m_supportedObjsWeb.begin(), this->m_supportedObjsWeb.end(), this->m_jsonSets.asArray().unwrap().at(it).get<int, std::string>("type").unwrap()) != this->m_supportedObjsWeb.end() && this->m_jsonSets.asArray().unwrap().at(it) != this->m_jsonSets.asArray().unwrap().at(0) && !isTransparent) {
                        isTransparent = true;
                        transparency = (float)this->m_jsonSets.asArray().unwrap().at(it)["color"].asArray().unwrap().at(3).asInt().unwrap() / 255;
                    }
                    if (this->m_jsonSets.asArray().unwrap().at(it).get<int, std::string>("type").unwrap() == this->m_supportedObjsWeb[0]) {
                        this->m_objsString << "1," << this->m_circle_id;
                        ...
                        this->m_objsString << ";";
                        z_order++;
                    } else if (this->m_jsonSets.asArray().unwrap().at(it).get<int, std::string>("type").unwrap() == this->m_supportedObjsWeb[1]) {
                        this->m_objsString << "1," << this->m_circle_id;
                        this->m_objsString << ",2," << (float)this->m_jsonSets.asArray().unwrap().at(it)["data"].asArray().unwrap().at(0).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionX();
                        this->m_objsString << ",3," << (float)this->m_jsonSets.asArray().unwrap().at(it)["data"].asArray().unwrap().at(1).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionY();
                        ...
                        this->m_objsString << ";";
                        z_order++;
                    } else if (this->m_jsonSets.asArray().unwrap().at(it).get<int, std::string>("type").unwrap() == this->m_supportedObjsWeb[2]) {
                        this->m_objsString << "1," << this->m_circle_id;
                        this->m_objsString << ",2," << (float)this->m_jsonSets.asArray().unwrap().at(it)["data"].asArray().unwrap().at(0).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionX();
                        this->m_objsString << ",3," << (float)this->m_jsonSets.asArray().unwrap().at(it)["data"].asArray().unwrap().at(1).asDouble().unwrap() * this->m_drawScale + this->m_centerObj->getPositionY();
                        this->m_objsString << ",128," << (float)this->m_jsonSets.asArray().unwrap().at(it)["data"].asArray().unwrap().at(2).asDouble().unwrap() * this->m_drawScale / 4;
