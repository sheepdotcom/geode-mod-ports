...
SetVolumenPopup* SetVolumenPopup::create(Slider* m_pSliderRef)
{
	...
	if (node && node->initAnchored(200.f, 100.f, m_pSliderRef))
	{
		node->autorelease();
	}
	else
...
bool SetVolumenPopup::setup(Slider* m_pSliderRef)
{
	...
	m_buttonMenu->setPosition({ m_mainLayer->getContentWidth() / 2.f + 2.f, m_mainLayer->getContentHeight() / 2.f + 100.f });
	...
	titleLayer->setPosition(m_mainLayer->getContentWidth() / 2.f, m_mainLayer->getContentHeight() / 2.f + 35.f);
	m_mainLayer->addChild(titleLayer);

	auto underline = cocos2d::CCSprite::createWithSpriteFrameName("floorLine_001.png");
	underline->setPosition({ m_mainLayer->getContentWidth() / 2.f, m_mainLayer->getContentHeight() / 2.f + 18.f });
	...
	m_pBGInputTextValue->setPosition({ m_mainLayer->getContentWidth() / 2.f - 40.f, m_mainLayer->getContentHeight() / 2.f - 14.f });
	...
	m_pInputTextValue->setPosition({ m_mainLayer->getContentWidth() / 2.f - 41.f, m_mainLayer->getContentHeight() / 2.f - 14.f });
	...
	perSymbol->setPosition({ m_mainLayer->getContentWidth() / 2.f + 9.f, m_mainLayer->getContentHeight() / 2.f - 15.f });
