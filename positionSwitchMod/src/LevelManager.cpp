...
LevelManager::LevelManager() {
    ...
    m_listener.bind([this] (web::WebTask::Event* e) {
        if (web::WebResponse* res = e->getValue()) {
            ...
            auto result = matjson::parse(_result.substr(0, _result.length() - 2)).unwrap();
            auto rows = result["table"]["rows"].asArray().unwrap();
            for (auto& row : rows) {
                auto contents = row["c"].asArray().unwrap();
                
                auto levelName = geode::utils::string::toLower(contents[0]["v"].asString().unwrap());
                auto levelID = contents[1]["f"].asString().unwrap();
